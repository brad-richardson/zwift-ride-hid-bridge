...
- Ride Left does not go quiet when released. Measured on hardware it advertises at ~344 ms for about two minutes, steps down to a ~6 s interval, then stops near three minutes. The bridge therefore does not wait for silence: it waits for the controller to leave fast advertising, then reconnects on a burst of fast advertisements, which only a wake or reboot produces. `Ride Advertising` shows which state the controllers are in, and the `Reconnect Ride Controllers` button overrides the whole thing.
