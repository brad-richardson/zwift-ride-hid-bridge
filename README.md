...
- After an idle period the bridge releases the Ride link so the controllers can sleep instead of being held awake by the connection, and by default disconnects the bonded HID host too so iPadOS gets its on-screen keyboard back. It then waits for their advertisement to disappear and return — a real sleep and wake — before reconnecting, with a cap so a controller that never sleeps cannot strand the bridge, and a `zwift_ride_hid.reconnect` action for getting the session back on demand. Every interval is configurable, and `disconnect_after: 0s` disables the feature.
- `Ride Advertising` and `Ride Advertisement Age` diagnostics report whether the controllers are still broadcasting, which is what decides when a reconnect is possible. `debug_advertisements` logs each advertisement's manufacturer payload, flags, and interval for tuning.
...
- A quiet session ends deliberately: after `idle_timeout.disconnect_after`, the bridge releases the Ride link and stops advertising the keyboard. It refuses to reconnect until the controllers have stopped advertising for `sleep_confirmation` and then advertise again. A held control counts as use and never triggers the timeout; lever movement below the press threshold does not.
- Because that refusal is deliberate, a button press on a controller that never stopped advertising will **not** reconnect. `Ride Advertising` tells you which situation you are in, and the `Reconnect Ride Controllers` button overrides it unconditionally. Hardware so far suggests the controllers keep advertising for a long time after release, so expect the button or a power-cycle to be the practical way back until `sleep_confirmation` is tuned from measured data.
