...
DeltaCore's bundled map uses arrows for the D-pad; `x`, `z`, `s`, and `a` for A, B, X, and Y; `q` and `w` for L1 and R1; Return for Start; Tab for Select; and Escape or `p` for Menu. A saved Delta mapping takes precedence for that system/controller pair.
...
| `right_side_lower` | RB / right drop button (`POWERUP_R`) | Escape | Delta menu/pause |
